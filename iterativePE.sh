...
for i in $(seq 0 $MAX_ITER); do
  ...
  # Map reads (single-end) and add read group info
    bowtie2 -x "$INDEX_PREFIX" -1 "$1" -2 "$name"*R2.fastq.gz -p $THREADS \
    ...
    #Fix mate information (required for markdup)
    samtools fixmate -m "$OUTDIR/${name}_mapped.queryname.bam" "$OUTDIR/${name}_mapped.fixmate.bam"

    # Coordinate sort
    samtools sort -@ $THREADS -o "$OUTDIR/${name}_mapped.coord.bam" "$OUTDIR/${name}_mapped.fixmate.bam"
